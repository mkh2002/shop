...
import { updateProduct } from "@/app/acitons/product";
...
const EditProductForm = ({ setOpen, row }: EditProductForm) => {
  ...
  const onSubmit = async (value: ProductType) => {
    try {
      const res = await updateProduct(product.id as string, value);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Edit a product</CardDescription>
      </CardHeader>
